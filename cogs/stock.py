...
class Stock(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.owners = [984245773887766551, 734416913437950011, 940215499764146176, 758957941033402388, 776029514811441152,
                       837584356988944396]
    ...
    async def add_stock(self, interaction, name:str, price:int):
        ...
        if interaction.user.id not in self.owners:
            await interaction.response.send_message("What colors are your IMO, NSO, IEO, IIO, IoE, IoS and GK medals this year? Thought so.")
            return

        self.stockmarket.add_stock(name, price)
        await interaction.response.send_message(f"`{name}` was added to the stock market successfully at the price `${price}` per stock")
